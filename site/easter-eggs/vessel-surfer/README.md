...
- **3D map** (`src/navigation-map.js`): the game renderer draws a second view into the map window through a scissor rectangle on the same canvas, so the map shows the real vessel surface rather than a line sketch. The route framing follows the player heading-up and keeps the player and destination in view; the whole-brain framing shows the full vasculature. A cyan cone is the player, the gold line and sphere are the route and destination, and the white dot is the start. The map has no fog and its own markers; the tunnel-only trail, beacon and headlamp are hidden during that pass.
...
The challenge uses a fixed destination 12 mm along a connected route from the same launch point in the widest long arterial trunk. Cruising speed is six voxels per second at 1×, so it scales with the data's resolution; the slider defaults to 0.5× and is remembered on the device under `vessel-surfer.speed.v1`. Points are `max(0, 10000 - ceil(activeSeconds * 20) - wallBumps * 400)`. Time comes from the monotonic browser clock, independent of the simulation's frame-time cap. Pausing or hiding the tab stops the timer. Holding against a wall is one contact; moving away rearms the bump counter.
