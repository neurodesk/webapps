...
- **Computer**: WASD or arrows turn and pitch, R turns around, Shift brakes, B reverses, Space or Escape pauses. The mouse steers only while its button is held: click and hold anywhere to raise a floating joystick under the pointer and drag; release to fly straight. A resting or moving pointer never steers, so it is free for the on-screen controls.
...
- A lumen assist (`src/assist.js`) probes five rays ahead and nudges toward the more open side of the vessel with the authority the player is not using. It never pushes against a deliberate input (inputs below a small threshold do not count) and is off while braking or reversing. Pinned on a wall, it probes 60 degrees round instead of 30, takes full strength, and only a firm input overrides it, to help negotiate sharp bends. If the sub makes less than 0.15 voxel of progress for two seconds without deliberate input, the movement controller checks swept clearance in 65 directions, including behind it, holds a safe escape heading, and moves away slowly before restoring normal assist. Steering, braking, reversing or a U-turn cancels recovery immediately. All recovery movement uses the same wall and eye-clearance checks as manual movement.
...
Unit tests densely sample every real route against the vessel field, exercise camera containment over thousands of movement steps, reject exterior camera positions, cover junction traversal, the steering model, the U-turn, the lumen assist, a captured Narrows pose, symmetric wall-stall recovery at multiple frame rates, manual recovery cancellation, roll levelling, and the leaderboard client with and without a server. Browser tests follow the homepage link, exercise the menu states on desktop and phone, keyboard steering, that a moving mouse neither steers nor blocks the in-run speed slider while a held drag does steer, the R key U-turn, tilt steering through synthetic `deviceorientation` events with tap-to-recentre, touch hold buttons through CDP touch events, the map and its framing toggle, timer, mocked global and offline leaderboards, the absence of file-upload controls, and complete runs driven by a keyboard autopilot.

The embed is assembled at `/surf/` with noindex metadata. The build downloads SHA-256-verified data from the immutable Hugging Face revision in `data-manifest.json`; the source NIfTI is retained there for provenance and is omitted from the deployed site. Rendering uses [Three.js](https://threejs.org/).
