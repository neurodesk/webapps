...
## Custom element migration, 18 September 2026

Shared consoles, file fields, result lists, viewer toolbars and example selectors
are light-DOM custom elements. All existing builder callers and the canonical
template use the returned element directly. Native controls, shared classes,
scientific pipelines and the application bar retain their existing owners.

Independent instances have separate generated control IDs. Example selectors
listen for replacement uploads within their workflow. Moving an element preserves
its controls; permanent removal releases observers/listeners and cancels example
loads. Static apps explicitly scope uploads to their `.app-container`.

Agent review found and corrected disconnected console state, toolbar calls before
mounting, viewer readiness guards, disabled-file-field state, HTML-label accessible
