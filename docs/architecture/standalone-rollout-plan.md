...
All 24 catalog apps get a Standalone action with ready-to-run GitHub downloads and a verified Neurodesk container option where one exists. The primary desktop deliverable is one Neurodesk Webapps Electron application containing the entire catalog. Individual Electron apps are generated subsets of the same build. Each batch app also gets an executable command for HPC. Every distribution includes its required runtime assets. The models ship once as a separate platform-independent pack, so a fresh installation works without downloads as soon as that pack is extracted and named in `NEURODESK_MODELS_DIR`. Source compilation instructions do not satisfy standalone delivery.
...
Keep model sources and validation datasets on the pinned Hugging Face dataset, and keep generated runtime output out of Git. CI fetches and verifies assets while building the release. The desktop, CLI and container artifacts include the actual templates, atlases, external tensor files and runtime dependencies for every supported method. Deduplicate only byte-identical assets by hash. The models are byte-identical across the four platform archives, so they are published once as a content-addressed pack instead of four times. Installation and the first scientific run must work with networking disabled and empty caches once that pack is extracted and `NEURODESK_MODELS_DIR` points at it. An HPC site can bind-mount one shared read-only pack into every job. Beyond the pack there is no setup-time model download or missing-asset network fallback. Bundle JavaScript, WASM, Python wheels and all other execution dependencies. ZARRo may use remote datasets by explicit choice; local Zarr input works offline.
...
3. Show the complete desktop suite and individual app downloads with explicit platform/version and checksum links. Include the app's HPC command package and Neurodesk instructions, distinguishing the same pipeline from upstream alternatives. Offer the model pack as a clearly optional extra for offline use. Keep developer build instructions out of this user flow.
...
5. Publish one desktop distribution per platform, the shared model pack, and the CLI archives and SIF images. Follow the shared architecture's multipart offline distribution design if a complete artifact exceeds GitHub's per-file limit.
...
- Test installation and first execution with networking disabled, empty caches and no developer environment. All required runtime dependencies are included, and the extracted model pack supplies every model. Run scientific fixtures, verify errors and outputs, and exercise paths containing spaces. GUI apps pass real import/edit/export workflows. Headless HPC commands pass without a display. Unexpected network attempts fail the offline check.
...
4. Add a changeset and run the repository release command for the app. App releases do not increment the desktop suite version. The `standalone` workflow runs only on its daily cron schedule at 03:23 UTC. It builds every app from the default branch snapshot selected for that run and publishes after all three installed desktop packages, the HPC image and the offline model pack pass their gates. Every job uses the UTC date selected by the bundle job as the desktop version patch; the committed major and minor versions define the release series. Pushes, pull requests and individual app releases do not build Electron.
...
6. Deployments and individual web releases proceed independently of the daily desktop suite. The imported standalone catalog records the versions in the last imported suite, which can trail the webapps. `scripts/desktop/check-published.mjs` remains an optional strict parity check, not a release or deployment gate. New apps still require standalone registry entries and offline assets so the next daily suite includes them.
