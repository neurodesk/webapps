...
The shared app bar owns the app identity, version, About, Cite, Privacy, Support, theme switch, More Apps and app source link. Support opens a GitHub issue that is already filled in with the app, build, browser and capability details and asks the reporter for reproduction steps or a feature suggestion; apps neither build that link nor add their own feedback control. Keep one visible bar in each screen. Avoid repeating these actions in the sidebar or footer. Apps that distribute a command-line package may also register an optional Standalone action in the same bar; put its installation and run instructions in the dialog it opens rather than in the workflow sidebar.
...
Every app declares a nonempty `apps/<id>/examples.json` array, except SeedSeg. SeedSeg uses an empty array and has no example selector because its synthetic example was withdrawn as unsuitable. There is no legacy exemption. Each example contains `id`, `label`, `description`, `expectedResult`, and `files`. Each file declares a semantic `role`, a unique filename `name`, and its pinned `url`. Multi-input workflows declare complete bundles, including gradients, acquisition metadata, masks, or reference images as needed. Explain what users can do with the result. A screenshot, model file, or link to browse an archive is not a runnable example.
...
For apps with examples, create one `nd-example-selector` with `createExampleSelector` from `@neurodesk/webapp-components/ui` in the open input section. Place it before the file picker so users see the example first and choose their own image second; the interface audit checks this order. It supplies the **Example** label, `select[data-neurodesk-example]`, loading status, Cancel, and retry behavior. Do not build another picker or load examples automatically on startup. The shared component owns downloads and aborts. The app's `onLoad(example, { signal, fetchFiles, assertCurrent })` imports the bundle through its normal input path, preserves the chosen algorithm, and resolves only after usable inputs are loaded. Call `assertCurrent()` after asynchronous preparation and before committing input state. Propagate failures. Selection enables the next action without automatically processing. For a viewer, opening the dataset is the action. Set its `scope` to the workflow containing replacement uploads. Removing it cancels active downloads after synchronous DOM moves have settled; `destroy()` cancels immediately.
...
Keep `ci.browser_test: true` and a runnable `test:e2e` script. For apps with examples, the interface audit requires exactly one visible, labelled shared selector, matching manifest options and successful first-example import on desktop and phone. For SeedSeg, it requires no example selector. It no longer assumes every scientific workflow has `#fileInfo` and `#runButton`. App-specific tests must also select an example, perform the real main action and verify a useful output or download. Test cancellation, failed downloads and retry. Exercise hosted bytes as well as mocked failures; a fixture-only test cannot prove real data suitability.
