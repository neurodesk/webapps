...
const results = createResultList({
  ...
  onDownload: (key) =>
    $("#downloads").querySelector(`[data-output="${key}"]`)?.click(),
});

// ---- parameter specs -------------------------------------------------------
const MP_SPEC = [
  ["tr", "TR (s)", 4.3, 5.0, 0.001],
  ["ti1", "TI1 (s)", 0.84, 0.7, 0.001],
  ["ti2", "TI2 (s)", 2.37, 2.5, 0.001],
  ["fa1", "flip 1 (°)", 5, 4, 0.1],
  ["fa2", "flip 2 (°)", 6, 5, 0.1],
  ["nz1", "NZslices 1", 64, 64, 1],
  ["nz2", "NZslices 2", 128, 128, 1],
  ["trflash", "TRFLASH (s)", 0.007, 0.0067, 0.0001],
  ["inveff", "inversion eff.", 0.96, 0.96, 0.01],
];
const SA_SPEC = [
  ["tr", "TR (s)", 2.4, 2.4, 0.001],
  ["ti1", "TD1 (s)", 0.15, 0.15, 0.001],
  ["ti2", "TD2 (s)", 1.5, 1.5, 0.001],
  ["fa1", "flip 1 (°)", 6, 6, 0.1],
  ["fa2", "flip 2 (°)", 6, 6, 0.1],
  ["nz1", "NZslices 1", 24, 24, 1],
  ["nz2", "NZslices 2", 24, 24, 1],
  ["trflash", "TRFLASH (s)", 0.005, 0.005, 0.0001],
  ["avgt1", "average T1 (s)", 1.5, 1.2, 0.05],
];
const B1_SPEC = [
  ["type", "map type", "tfl", "tfl"],
  ["refangle", "ref flip (°)", 80, 80, 1],
];
...
function buildForm(grid, spec, prefix) {
  ...
  for (const [id, label, d7] of spec) {
    ...
    f.className = "nd-field";
    const lab = document.createElement("label");
    lab.textContent = label;
    let inp;
    if (id === "type") {
      inp = document.createElement("select");
      for (const t of ["tfl", "percent", "relative"]) {
...
function applyPreset(which) {
  for (const [id, , , d3] of MP_SPEC) {
    const e = $(`#mp_${id}`);
    if (e) e.value = which === "3T" ? d3 : MP_SPEC.find((s) => s[0] === id)[2];
  }
  for (const [id, , , d3] of SA_SPEC) {
    const e = $(`#sa_${id}`);
    if (e) e.value = which === "3T" ? d3 : SA_SPEC.find((s) => s[0] === id)[2];
  }
...
function guessRole(name) {
  ...
  if (/tb1tfl|b1map|_b1\b|tfl.?b1|b1.?map/.test(n))
    return /anat/.test(n) ? "(ignore)" : "B1 map";
  return "(ignore)";
}
...
async function addFiles(fileList) {
  if (appMode === "bids") {
    log('In BIDS mode, switch to "Single dataset" to load individual files.');
    return;
  }
  for (const file of fileList) {
    ...
    if (lower.endsWith(".json")) {
      try {
        ...
        log(`could not parse ${file.name}: ${e}`);
      }
      continue;
    }
    if (!/\.nii(\.gz)?$/.test(lower)) {
      log(`skipped ${file.name} (not .nii/.nii.gz/.json)`);
      continue;
    }
    try {
      ...
      log(`failed to read ${file.name}: ${e}`);
    }
  }
  renderTable();
  refreshRunState();
...
function applySidecar(name, js) {
  ...
  if (/inv-?1|inv1/.test(n)) {
    set("#mp_ti1", js.InversionTime);
    set("#mp_fa1", js.FlipAngle);
  }
  if (/inv-?2|inv2/.test(n)) {
    set("#mp_ti2", js.InversionTime);
    set("#mp_fa2", js.FlipAngle);
  }
  if (/mp2rage|uni|inv/.test(n) && js.RepetitionTime)
    set("#mp_tr", js.RepetitionTime);
  $("#paramSrcNote").textContent = "from sidecars";
}

function reloadFromJsons() {
  if (!state.jsons.length) {
    log("no JSON sidecars loaded, drop the .json files too");
    return;
  }
...
$("#paramSource").onchange = () => {
  ...
  if (v === "json") reloadFromJsons();
    ...
    $("#paramSrcNote").textContent =
      v === "dicom" ? "from DICOM headers" : "manual";
};
...
function renderTable() {
  ...
  for (const [i, f] of state.files.entries()) {
    ...
    nameTd.innerHTML = `👁 <span>${esc(f.name)}</span><p class="nd-hint">${f.dims.join(" × ")}</p>`;
    ...
    b.className = "nd-btn nd-btn-secondary nd-btn-icon";
    b.textContent = "✕";
  ...
  for (const [i, j] of state.jsons.entries()) {
    ...
    tr.innerHTML = `<td>📄 ${esc(
      j.name
    )}</td><td class="nd-hint">JSON parameters</td>`;
    const del = document.createElement("td");
    const b = document.createElement("button");
    b.className = "nd-btn nd-btn-secondary nd-btn-icon";
    b.textContent = "✕";
    b.setAttribute("aria-label", "Remove " + j.name);
    b.onclick = () => {
      state.jsons.splice(i, 1);
      renderTable();
    };
    del.appendChild(b);
    tr.appendChild(del);
  ...
  $("#filetable").hidden = state.files.length === 0 && state.jsons.length === 0;
  renderWarnings();
}

function byRole(r) {
  return state.files.find((f) => f.role === r);
}
...
function imageTypeTokens(f) {
  ...
  const j = state.jsons.find(
    (x) => x.name.replace(/\.json$/i, "").toLowerCase() === base
  );
  if (j && Array.isArray(j.json.ImageType))
    return j.json.ImageType.map((s) => String(s).toUpperCase().trim());
  return null;
}
...
function renderWarnings() {
  ...
  const byrole = {};
  for (const f of state.files)
    if (f.role !== "(ignore)") (byrole[f.role] ||= []).push(f);
  ...
  for (const f of state.files) {
    ...
    const looksAnat =
      /b1map|tb1tfl|tfl.?b1/i.test(f.name + " " + (f.seriesDesc || "")) &&
      t &&
      !isFAM &&
...
function refreshRunState() {
  const uni = byRole("UNI"),
    inv1 = byRole("INV1"),
    inv2 = byRole("INV2"),
    sa = byRole("SA2RAGE"),
    b1 = byRole("B1 map");
  const task = $("#taskSel").value;
  $("#regField").hidden = task !== "denoise";
  ...
  if (task === "denoise") {
    ...
    label = "Denoise UNI";
    status = ok
      ? "Ready: robust-combination denoising (UNI + INV1 + INV2)."
      : "Denoise needs UNI, INV1 and INV2.";
    $("#saBlock").hidden = true;
    $("#b1Block").hidden = true;
  } else if (task === "b1only") {
    mode = "sa2rage";
    ...
    status = ok
      ? "Ready: relative B1 map from SA2RAGE."
      : "SA2RAGE → B1 needs UNI, INV2 and SA2RAGE.";
    $("#saBlock").hidden = false;
    $("#b1Block").hidden = true;
  } else {
    mode = sa ? "sa2rage" : b1 ? "b1map" : null;
    ok = !!(uni && mode);
    label = "Compute T1 map";
    $("#saBlock").hidden = mode !== "sa2rage";
    $("#b1Block").hidden = mode !== "b1map";
    status = ok
      ? `Ready: ${mode === "sa2rage" ? "SA2RAGE" : "B1-map"} correction${
          inv2 ? "" : " (no INV2 → mask from UNI)"
        }.`
  ...
  if (!running) $("#statusText").textContent = status;
...
async function addDicomFolder(name, files) {
  if (appMode === "bids") {
    log('In BIDS mode, switch to "Single dataset" to load a DICOM folder.');
    return;
  }
  ...
  const concat = new Uint8Array(total),
    offsets = new Uint32Array(bufs.length + 1);
  let o = 0;
  bufs.forEach((b, i) => {
    offsets[i] = o;
    concat.set(new Uint8Array(b), o);
    o += b.byteLength;
  });
  offsets[bufs.length] = o;
  let v;
  try {
    v = parse_dicom_series(concat, offsets);
  } catch (e) {
    log(`  ${name}: ${e}`);
    return;
  }
  ...
  state.files.push({
    ...
    seriesDesc: v.series_desc,
    src: { concat, offsets },
  });
  log(
    `  ${name} → [${dims.join("×")}] role ${v.role}${
      v.image_type ? "  [" + v.image_type + "]" : ""
    }`
  );
  ...
  if (params.length === 7) {
    [
      ...
      const el = $("#" + id);
      if (el) el.value = +params[i].toFixed(4);
    });
...
fileField.onFiles(async (files) => {
  exampleSelector?.cancel();
  await addPickedFolders(await files);
});
...
async function addPickedFolders(fileList) {
  if (running) {
    log("Stop processing before changing inputs.");
    return;
  }
  const groups = {};
  for (const f of [...fileList]) {
    const rel = f.webkitRelativePath || f._webkitRelativePath || f.name;
...
$("#folder").onchange = (e) => {
  exampleSelector?.cancel();
  void addPickedFolders(e.target.files);
};
...
function stopProcessing(msg) {
  ...
  if (bidsMode && bidsCurrent && bidsCurrent._state === "running") {
    bidsCurrent._state = "loaded";
    updateBidsStatus(bidsCurrent);
    updateBidsActions(bidsCurrent);
  }
...
$("#stop").onclick = () =>
  stopProcessing("■ stopped, inputs and parameters kept. Ready to run again.");

// Full reset: clear all inputs, parameters, results and logs back to defaults.
function resetAll() {
  if (
    (state.files.length || state.jsons.length) &&
    !confirm(
      "Reset everything? This clears all loaded images, JSON sidecars, parameters and results."
    )
  )
    return;
  stopProcessing();
  ...
  appMode = "single";
  applyModeVisibility(); // back to the default input mode
  state.files = [];
  state.jsons = [];
  exampleSelector?.cancel();
  results.render();
  ...
  buildForm($("#b1Grid"), B1_SPEC, "b1");
  $("#paramSource").value = "manual";
  $("#paramSrcNote").textContent = "manual";
  $("#taskSel").value = "t1";
...
function validateBeforeRun(sel) {
  ...
  if (mode === "sa2rage" && sa && (sa.dims[3] || 1) < 2)
    return `SA2RAGE must be a 2-volume (S1,S2) image, this one is ${sa.dims
      .slice(0, 4)
      .join("×")}. Load the original 2-volume SA2RAGE.`;
  const bad = [];
  const chk = (names, vals) =>
    names.forEach((n, i) => {
      if (!Number.isFinite(vals[i])) bad.push(n);
    });
  if (task !== "denoise")
    chk(
      ["TR", "TI1", "TI2", "FA1", "FA2", "NZ1", "NZ2", "TRFLASH", "invEff"],
      mpParams()
    );
  if (mode === "sa2rage")
    chk(
  ...
  if (mode === "b1map" && !Number.isFinite(num("#b1_refangle")))
    bad.push("ref flip");
  if (task === "denoise" && !Number.isFinite(num("#reg")))
    bad.push("denoise strength");
  if (bad.length)
    return `These parameter fields are empty or not a number: ${bad.join(
      ", "
    )}. Fill them in (or use a preset).`;
  return null;
}
...
$("#run").onclick = async () => {
  ...
  if ($("#verbose")?.checked) {
    const roles = state.files
      ...
      .join(", ");
    log(
      `  [verbose] task=${task}  mode=${mode || "-"}  grid=${dims.join("×")}`
    );
    log(`  [verbose] roles: ${roles || "(none)"}`);
    log(
      `  [verbose] MP2RAGE [TR,TI1,TI2,FA1,FA2,NZ1,NZ2,TRFLASH,invEff] = ${mpParams()
        .map((x) => +x.toFixed(4))
        .join(", ")}`
    );
    if (mode === "sa2rage")
      log(
        `  [verbose] SA2RAGE [TR,TD1,TD2,FA1,FA2,NZ1,NZ2,TRFLASH,avgT1] = ${saParams()
          .map((x) => +x.toFixed(4))
          .join(", ")}`
      );
    if (mode === "b1map")
      log(
        `  [verbose] B1 map: kind=${$("#b1_type").value} refAngle=${num(
          "#b1_refangle"
        )}° extend-FOV=${$("#extendFov")?.checked}`
      );
    if (task === "denoise")
  ...
  if (task === "denoise") {
    ...
    const inv1Copy = inv1.data.slice(),
      inv2Copy = inv2.data.slice();
    const msg = {
      mode: "denoise",
      uni: uniCopy,
      inv1: inv1Copy,
...
async function onResult(res, uni, task, mode, t0) {
  ...
  if (res.type === "error") {
    ...
    if (full.includes("\n")) {
      if ($("#verbose")?.checked) log(full);
      else log("  (turn on Verbose log for the full error)");
    }
    if (bidsMode && bidsCurrent) {
      bidsCurrent._state = "error";
      updateBidsStatus(bidsCurrent);
      updateBidsActions(bidsCurrent);
    }
    $("#progress").hidden = true;
  ...
  if (res.type === "progress") {
    $("#statusText").textContent = res.stage || "Processing…";
    setProgress(res.pct);
    if (res.stage && $("#verbose")?.checked) log("  · " + res.stage);
    return;
  }
  ...
  if (task === "denoise") {
    outputs.unic = {
      ...
      dims,
      affine: aff,
      label: "UNI-DEN (denoised)",
      range: [0, 4095],
      cmap: "gray",
    };
    views = [
      ["unic", "UNI-DEN (denoised)"],
      ["uni", "UNI (input)"],
    ];
  } else {
    outputs.t1 = {
  ...
  if ($("#verbose")?.checked) {
    for (const [k, o] of Object.entries(outputs)) {
      ...
      log(
        `  [verbose] ${o.label}: ${
          s.nz
        } non-zero voxels, median ${s.med.toFixed(
          k === "b1" ? 3 : 0
        )}, range [${s.min.toFixed(0)}, ${s.max.toFixed(0)}]`
      );
    }
  }
  setProgress(100);
...
async function buildDownloads(task, mode, uni) {
  ...
  results.render(
    Object.fromEntries(
      items
        ...
        .map(([key]) => [key, { description: outputs[key].label }])
    )
  );
  ...
  if (task === "t1" && uni && uni.src && outputs.t1) {
    try {
      ...
      log(
        deid
          ? "  DICOM: output will be de-identified (patient tags removed)"
          : "  DICOM: output keeps source patient tags (de-identify unchecked)"
      );
      const data = dout.data,
        offs = dout.offsets,
        files = [];
      for (let i = 0; i < offs.length - 1; i++)
        files.push({
          name: `T1map_${String(i + 1).padStart(4, "0")}.dcm`,
          data: data.subarray(offs[i], offs[i + 1]),
        });
      addLink(
        dd,
      ...
      for (const f of files)
        bundle.push({ name: `T1map_DICOM/${f.name}`, data: f.data });
  ...
  if (bundle.length > 1) {
    const a = addLink(
      dd,
      new Blob([zipStore(bundle)], { type: "application/zip" }),
      "all_derivatives.zip",
      true
    );
...
function addLink(parent, blob, fname, savesResult = false) {
  ...
  a.href = url;
  a.download = fname;
  a.className = "nd-btn nd-btn-secondary";
  a.textContent = "⬇ " + fname;
  ...
  if (savesResult)
    a.addEventListener("click", () => {
      if (bidsMode && bidsCurrent && bidsCurrent._state === "done") {
        bidsCurrent._downloaded = true;
        updateBidsActions(bidsCurrent);
      }
    });
...
const CMAPS = {
  ...
  hot: (t) => [
    Math.min(1, 3 * t) * 255,
    Math.min(1, Math.max(0, 3 * t - 1)) * 255,
    Math.min(1, Math.max(0, 3 * t - 2)) * 255,
  ],
  viridis: ramp([
    [68, 1, 84],
    [59, 82, 139],
...
function volStats(data) {
  const n = data.length,
    step = Math.max(1, Math.floor(n / 300000));
  const nzv = [];
  let nz = 0,
    min = Infinity,
    max = -Infinity;
...
function previewInput(f, includeResultViews = true) {
  if (!f || !f.data || !f.data.length) {
    log(`cannot preview ${f?.name || "?"} (no data, re-add the file)`);
    return;
  }
  ...
  outputs.__preview = {
    ...
    affine: f.affine,
    label: "INPUT · " + f.name,
    range: robustRange(f.data),
    cmap: "gray",
  };
  const label =
    "INPUT · " +
    f.name +
    (f.role && f.role !== "(ignore)" ? ` (${f.role})` : "");
...
function histSmooth(data, rlo, rhi, nb) {
  ...
  for (let i = 0; i < data.length; i++) {
    const v = data[i];
    if (v > rlo && v < rhi)
      bins[Math.min(nb - 1, (((v - rlo) / (rhi - rlo)) * nb) | 0)]++;
  }
  return bins.map(
    (_, b) =>
      (bins[Math.max(0, b - 1)] + 2 * bins[b] + bins[Math.min(nb - 1, b + 1)]) /
      4
  );
}
...
function drawHistogram(o) {
  ...
  const rlo = isT1 ? 400 : o.range[0],
    rhi = isT1 ? 3200 : o.range[1];
  const nb = 150;
  const pad = 20,
    top = 18;
  ...
  const series =
    isT1 && outputs.t1 && outputs.t1u
      ? [
          {
            data: outputs.t1u.data,
            color: "#f0b03a",
            fill: false,
            label: "uncorrected",
          },
          {
            data: outputs.t1.data,
            color: "#5aa2ff",
            fill: true,
            label: "B1-corrected",
          },
  ...
  let maxc = 0;
  for (const s of S) for (const v of s.sm) maxc = Math.max(maxc, v);
  if (maxc === 0) {
    ctx.fillStyle = "#93a1b0";
    ctx.fillText("no data in range", 12, H / 2);
    return;
  }
  ...
  for (const s of S) {
    ...
    for (let b = 0; b < nb; b++) {
      const x = xOf(binT1(b)),
        y = H - pad - (s.sm[b] / maxc) * (H - pad - top);
      b === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    }
    if (s.fill) {
      ctx.lineTo(xOf(binT1(nb - 1)), H - pad);
      ctx.lineTo(xOf(binT1(0)), H - pad);
      ctx.closePath();
  ...
  ctx.fillStyle = "#93a1b0";
  ctx.fillText(
    isT1 ? "T1 histogram (brain)" : `${o.label}, brain histogram`,
    pad,
    12
  );
  ctx.fillText(String(Math.round(rlo)), pad, H - 5);
  ctx.textAlign = "right";
  ctx.fillText(String(Math.round(rhi)), W - 5, H - 5);
  ctx.textAlign = "left";
...
function topTwoModes(sm, nb, binT1) {
  let maxc = 0;
  for (const v of sm) if (v > maxc) maxc = v;
  if (maxc === 0) return [];
  ...
  for (const b of cands) {
    if (picks.every((p) => Math.abs(binT1(p) - binT1(b)) > minSepMs))
      picks.push(b);
...
function b1SanityWarn(task) {
  ...
  for (let i = 0; i < n; i += step) {
    const x = d[i];
    if (x > 0 && Number.isFinite(x)) v.push(x);
  }
  if (v.length < 100) {
    w.hidden = true;
    return;
  ...
  const med = q(0.5),
    spread = med > 0 ? (q(0.9) - q(0.1)) / med : 99;
  ...
  const bad =
    med < 0.5 || med > 1.7 || (spread > 1.0 && (med < 0.75 || med > 1.3));
  if (bad) {
    const advise =
      task === "b1only"
    ...
    w.innerHTML =
      `<b>⚠ This B1 map does not look like a B1⁺ field</b> (relative B1 median ${med.toFixed(
        2
      )}; a real transmit field sits near 1.0 and varies smoothly). ` +
      `The input may be a magnitude/anatomical image rather than a flip-angle map, or the units/type are wrong. ` +
      advise;
    w.hidden = false;
    log(
      `⚠ B1 sanity: relative B1 median ${med.toFixed(
        2
      )} (expected ~1.0); this does not look like a B1⁺ field.`
    );
  } else {
    w.hidden = true;
  }
...
function showView(key) {
  for (const [index, button] of [
    ...$("#resultList").querySelectorAll(".nd-view-btn"),
  ].entries()) {
    const outputKey = [...$("#downloads").querySelectorAll("[data-output]")][
      index
    ]?.dataset.output;
    button.classList.toggle("active", outputKey === key);
    button.setAttribute("aria-pressed", String(outputKey === key));
  }
  const o = outputs[key];
  if (!o) return;
  curKey = key;
  if (o.cmap && CMAPS[o.cmap]) $("#cmapSel").value = o.cmap; // per-view default colormap
  $("#viewStat").textContent = `${o.label} · ${o.dims.join("×")} · window [${
    o.range[0]
  }, ${o.range[1]}]`;
  drawSlices(o);
  drawHistogram(o);
}
$("#histogramPanel").addEventListener("toggle", () => {
  if ($("#histogramPanel").open && outputs[curKey]) drawHistogram(outputs[curKey]);
});
...
function syncModeButtons() {
  ...
  $("#modeSingle").setAttribute("aria-pressed", appMode === "single");
  $("#modeBids").setAttribute("aria-pressed", appMode === "bids");
}
function applyModeVisibility() {
  const bids = appMode === "bids";
  $("#drop").hidden = bids;
  $("#dicomRow").hidden = bids;
  $("#bidsRow").hidden = !bids;
  syncModeButtons();
}
function setAppMode(mode) {
  ...
  if (running) {
    log("Stop the current run before switching modes.");
    return;
  }
  if (bidsMode) closeBids();
  revokeDownloadUrls();
  ...
  log(
    mode === "bids"
      ? "Switched to BIDS directory mode. Choose a BIDS dataset root."
      : "Switched to single-dataset mode."
  );
}
...
function bidsShowPlaceholder() {
  ...
  $("#emptyState").textContent =
    "Load or calculate a BIDS session to view it here.";
  $("#downloads").replaceChildren();
  results.render();
  $("#viewWarn").hidden = true;
}
...
function applyBidsSessionOptions(ses) {
  ...
  if ($("#verbose") && $("#bidsVerbose"))
    $("#verbose").checked = $("#bidsVerbose").checked;
}
...
$("#bidsVerbose").onchange = () => {
  if ($("#verbose")) $("#verbose").checked = $("#bidsVerbose").checked;
};
...
function loadBidsDirectory(fileList) {
  ...
  const entries = [];
  for (const f of [...fileList]) {
    const segs = (f.webkitRelativePath || f._webkitRelativePath || f.name)
      .replace(/\\/g, "/")
      .split("/");
    const si = segs.findIndex((s) => /^sub-[A-Za-z0-9]+$/.test(s));
    entries.push({
      path: si >= 0 ? segs.slice(si).join("/") : segs.join("/"),
      file: f,
    });
  }
  let idx;
  try {
    idx = indexBids(entries);
  } catch (err) {
    log("BIDS: could not index this directory: " + err);
    return;
  }
  if (!idx.nSubjects) {
    log("BIDS: no sub-* subjects found here. Point at a BIDS dataset root.");
    return;
  }
  bidsIndex = idx;
  bidsMode = true;
  ...
  for (const sub of idx.subjects)
    for (const ses of sub.sessions) {
      ses._sub = sub.id;
      ses._roles = { ...ses.roles };
      ses._state = "pending";
      bidsIndex._flat.push(ses);
    }
  const runnable = bidsIndex._flat.filter(
    (s) => s.runnable.t1 || s.runnable.b1only || s.runnable.denoise
  ).length;
  $(
    "#bidsSummary"
  ).textContent = `${idx.nSubjects} subject(s) · ${idx.nSessions} session(s) · ${runnable} runnable`;
  renderBidsTree();
  $("#bidsPanel").hidden = false;
  bidsShowPlaceholder();
  $("#bidsPanel").scrollIntoView({ behavior: "smooth", block: "start" });
  log(
    `BIDS: indexed ${idx.nSubjects} subject(s), ${idx.nSessions} session(s); ${runnable} runnable.`
  );
}
...
const roleLabel = (rk) =>
  ({ UNI: "UNI", INV1: "INV1", INV2: "INV2", B1SOURCE: "B1 source" }[rk] || rk);
function bidsInitSession(ses) {
  if (ses._task === undefined)
    ses._task = ses.runnable.t1
      ? "t1"
      : ses.runnable.b1only
      ? "b1only"
      : ses.runnable.denoise
      ? "denoise"
      : null;
  if (ses._editRole === undefined) ses._editRole = "UNI";
  // per-session processing options (seeded from the toolbar batch defaults)
  if (ses._extendFov === undefined)
    ses._extendFov = !!($("#bidsExtendFov") && $("#bidsExtendFov").checked);
  if (ses._fallback === undefined)
    ses._fallback = !!($("#bidsFallback") && $("#bidsFallback").checked);
  if (ses._reg === undefined)
    ses._reg = ($("#bidsReg") ? +$("#bidsReg").value : 6) || 6;
}
...
function bidsRecomputeRunnable(ses) {
  ...
  ses.runnable = {
    ...
    b1only:
      has("UNI") && has("INV2") && has("B1SOURCE") && ses.b1kind === "sa2rage",
  };
  ...
  ses.status = {
    ...
    [rk]: file
      ? roleFits(rk, file.path)
        ...
        : {
            state: "warn",
            reason: `manually assigned; ${baseName(
              file.path
            )} does not look like a ${roleLabel(rk)}`,
          }
  ...
  if (["loaded", "done", "unloaded"].includes(ses._state)) {
    if (ses._dlUrl) {
      ...
      if (
        !ses._downloaded &&
        confirm(
          `Reassigning ${roleLabel(
            rk
          )} discards the computed result for ${entryLabel(ses)} (${
            ses._dlName
          }), which has not been downloaded. Download it first?`
        )
      ) {
        bidsDownload(ses);
        triggered = true;
      ...
      if (triggered) deferRevoke(ses._dlUrl);
        ...
        try {
          URL.revokeObjectURL(ses._dlUrl);
        } catch (e) {
          /* */
        }
      }
...
function bidsChipClick(ses, rk, rlabel) {
  ...
  if (ses._el)
    ses._el
      ...
      .forEach((c) => c.classList.toggle("active", c.dataset.rk === rk));
  const rn = ses._el && ses._el.querySelector(".bids-reassign b");
  if (rn) rn.textContent = roleLabel(rk);
...
function renderBidsSession(ses) {
  ...
  box.className = "nd-section-content";
  ses._el = box;
  ...
  for (const [rk, rlabel] of BIDS_ROLES) {
    ...
    chip.type = "button";
    chip.className = "nd-btn nd-btn-secondary chip " + cls;
    chip.dataset.rk = rk;
    chip.textContent = `${rlabel}: ${
      chosen ? baseName(chosen.path) : "(none)"
    }`;
    if (rk === ses._editRole) chip.classList.add("active");
    chip.title =
      (st.reason ? st.reason + "  ·  " : "") +
      (chosen
        ? "click to preview and reassign this role"
        : "click to reassign this role");
  ...
  ctrl.className = "nd-field bids-reassign";
  const tasks = bidsTasks(ses);
  if (tasks.length) {
    const tl = document.createElement("label");
    tl.textContent = "Task: ";
    const tsel = document.createElement("select");
    for (const [v, lbl] of tasks) {
      const o = document.createElement("option");
      o.value = v;
      o.textContent = lbl;
      if (v === ses._task) o.selected = true;
      tsel.appendChild(o);
    }
    tsel.onchange = () => {
      ses._task = tsel.value;
      rerenderBidsSession(ses);
    }; // re-render so the task-specific options update
    tl.appendChild(tsel);
    ctrl.appendChild(tl);
  }
  ...
  if (ses._task === "t1") {
    if (ses.b1kind === "b1map")
      mkChk(
        ...
        "Extend a too-small B1 map to the whole brain (filled regions are estimates)."
      );
    mkChk(
      "fill non-converged",
      ses._fallback,
      ...
      "Fill non-converged T1 voxels (mostly CSF / long-T1) with the uncorrected T1."
    );
  }
  if (ses._task === "denoise") {
    const l = document.createElement("label");
    l.className = "nd-check";
    l.title = "Denoise strength (larger removes more background).";
    ...
    n.step = "1";
    n.min = "1";
    n.setAttribute("aria-label", "Denoise strength");
    n.oninput = () => {
      ses._reg = +n.value || 6;
    };
    l.appendChild(n);
  ...
  rl.className = "nd-field bids-reassign";
  rl.appendChild(document.createTextNode("Reassign "));
  const rn = document.createElement("b");
  rn.textContent = roleLabel(ses._editRole);
  rl.appendChild(rn);
  rl.appendChild(document.createTextNode(": "));
  const rsel = document.createElement("select");
  ses._reassignSel = rsel;
  fillReassign(ses, rsel);
  rsel.onchange = () =>
    bidsAssignRole(
      ses,
      ses._editRole,
      (ses.files || []).find((f) => f.path === rsel.value) || null
    );
  rl.appendChild(rsel);
  ctrl.appendChild(rl);
  ...
  act.className = "nd-field";
  ses._actEl = act;
  box.appendChild(act);
  const status = document.createElement("span");
  status.className = "bids-status";
  ses._statusEl = status;
  box.appendChild(status);
  updateBidsActions(ses);
  updateBidsStatus(ses);
  return box;
}
...
function updateBidsStatus(ses) {
  ...
  const m = {
    pending: "",
    loading: "⋯ loading…",
    loaded: "● loaded, press Compute",
    running: "⋯ processing…",
    done: "✓ done, result in the viewer",
    error: "✖ error",
    unloaded: "✓ saved & unloaded",
  };
...
function updateBidsActions(ses) {
  const act = ses._actEl;
  if (!act) return;
  act.innerHTML = "";
  const mk = (txt, fn) => {
    const b = document.createElement("button");
    b.type = "button";
    b.className = "nd-btn nd-btn-secondary";
    b.textContent = txt;
    b.onclick = fn;
    act.appendChild(b);
    return b;
  ...
  if (ses._state === "running") {
    const s = document.createElement("span");
    s.className = "bids-status";
    s.textContent = "processing… (Stop to cancel)";
    act.appendChild(s);
    return;
  }
  if (ses._state === "done") {
    dlLink();
    mk("👁 View", () => viewBidsResult(ses));
    mk("Download & unload", () => downloadUnloadBids(ses));
    return;
  }
  if (ses._state === "unloaded") {
    dlLink();
    mk("⚙ Recalculate", () => calculateBidsEntry(ses));
    return;
  }
  // pending / loaded / error
  mk("load & edit", () => loadBidsEntry(ses, { scroll: false }));
  if (!bidsTasks(ses).length) {
    const s = document.createElement("span");
    s.className = "bids-status";
    s.textContent = "not runnable (missing inputs)";
    act.appendChild(s);
    return;
  }
  const label =
...
async function previewBidsRole(ses, rk, rlabel) {
  ...
  if (!f) {
    log(`BIDS: no ${rlabel} file for ${entryLabel(ses)}.`);
    return;
  }
  const appRole =
    rk === "B1SOURCE" ? (ses.b1kind === "sa2rage" ? "SA2RAGE" : "B1 map") : rk;
  if (bidsCurrent === ses) {
    const got = state.files.find((x) => x.role === appRole);
    if (got && got.data && got.data.length) {
      previewInput(got);
      return;
    }
  }
  try {
    ...
    log(`BIDS: could not preview ${baseName(f.path)}: ${e}`);
  }
}
...
async function loadBidsEntry(ses, opts = {}) {
  if (running) {
    log("BIDS: a run is in progress, Stop it first.");
    return false;
  }
  if (bidsLoading) {
    log("BIDS: a session is still loading, one moment.");
    return false;
  }
  bidsLoading = true;
  ...
  if (bidsCurrent && bidsCurrent !== ses && bidsCurrent._state === "done") {
    ...
    if (
      ...
      !prev._downloaded &&
      confirm(
        `The result for ${entryLabel(prev)} (${
          prev._dlName
        }) has not been downloaded and will be freed to make room for the next session. Download it now?`
      )
    ) {
      bidsDownload(prev);
      triggered = true;
    ...
    if (prev._dlUrl) {
      ...
      if (triggered) deferRevoke(prev._dlUrl);
        ...
        try {
          URL.revokeObjectURL(prev._dlUrl);
        } catch (e) {
          /* */
        }
      }
    ...
    prev._state = "unloaded";
    updateBidsStatus(prev);
    updateBidsActions(prev);
  }
  bidsCurrent = ses;
  revokeDownloadUrls();
  clearOutputs();
  ...
  for (const s of bidsIndex._flat)
    if (s._el) s._el.classList.toggle("current", s === ses);
  ses._state = "loading";
  updateBidsStatus(ses);
  updateBidsActions(ses);
  try {
    ...
    for (const rk of ["UNI", "INV1", "INV2", "B1SOURCE"]) {
      ...
      if (rk === "B1SOURCE") {
        ...
        b1type = /tb1tfl/i.test(f.path)
          ? "tfl"
          : /tb1map|rb1cor/i.test(f.path)
          ? "percent"
          : "tfl";
      } else role = rk;
      const buf = await f.file.arrayBuffer();
    ...
    for (const sc of ses.sidecars || []) {
      try {
        state.jsons.push({
          name: baseName(sc.path),
          json: JSON.parse(await sc.file.text()),
        });
      } catch (e) {
        /* ignore */
      }
    ...
    const b0 = state.jsons
      .map((j) => parseFloat(j.json.MagneticFieldStrength))
    ...
    if (state.jsons.length) {
      ...
      $("#paramSrcNote").textContent =
        (b0 && b0 >= 6.5 ? "7T" : "3T") + " preset (no sidecars)";
    ...
    $("#taskSel").value =
      ses._task ||
      (ses.runnable.t1 ? "t1" : ses.runnable.denoise ? "denoise" : "t1");
    renderTable();
    refreshRunState();
    ses._state = "loaded";
    updateBidsStatus(ses);
    updateBidsActions(ses);
    log(
      `BIDS: loaded ${entryLabel(ses)} (${state.files
        .map((f) => f.role)
        .join(", ")}${
        state.jsons.length
          ? "; params from " + state.jsons.length + " sidecar(s)"
          : ""
      }).`
    );
...
async function calculateBidsEntry(ses) {
  if (running) {
    log("BIDS: a run is already in progress, Stop it first.");
    return;
  }
  if (bidsLoading) {
    log("BIDS: a session is still loading, one moment.");
  ...
  if ($("#run").disabled || err) {
    log(
      `BIDS: cannot calculate ${entryLabel(ses)}${
        err ? ": " + err : " (missing inputs)."
      }`
    );
    ses._state = "error";
    updateBidsStatus(ses);
...
function roleFits(rk, path) {
  ...
  if (rk === "B1SOURCE")
    return /tb1tfl|tb1srge|tb1map|rb1cor|b1|fmap|sa2rage/.test(b);
  return true;
}
...
async function bidsRunComplete(task) {
  ...
  if (o) {
    ...
    ses._dlUrl = URL.createObjectURL(
      new Blob([gz], { type: "application/gzip" })
    );
    ses._dlName = fname;
    ses._downloaded = false;
    if ($("#bidsAutoDl")?.checked) {
      ...
      log(
        `BIDS: ${entryLabel(
          ses
        )} done → ${fname}. Review it, then download it (⬇ on the row) before the next session.`
      );
  }
  updateBidsActions(ses);
...
function viewBidsResult(ses) {
  if (bidsCurrent !== ses || (!outputs.t1 && !outputs.unic)) {
    log("BIDS: result no longer in memory, Recalculate to view.");
    return;
  }
  setViewerVisible(true);
  showView($("#viewSel").value);
...
function downloadUnloadBids(ses) {
  ...
  bidsDownload(ses);
  if (ses._dlUrl) {
    if (had) deferRevoke(ses._dlUrl);
    ses._dlUrl = null;
  } // saved; free the blob after the download task grabs it
  clearOutputs();
  state.files = [];
  state.jsons = [];
  lastViews = null;
...
function calculateNextBidsEntry() {
  if (!bidsIndex) {
    log("BIDS: no dataset loaded.");
    return;
  }
  if (running) {
    log("BIDS: a run is in progress, wait for it or Stop.");
    return;
  }
  ...
  for (let n = 0; n < list.length; n++) {
    ...
    if (
      (s.runnable.t1 || s.runnable.b1only || s.runnable.denoise) &&
      s._state !== "done" &&
      s._state !== "unloaded"
    ) {
      if (s._el) {
        const d = s._el.closest("details");
        if (d) d.open = true;
      }
...
refreshRunState();
log(
  "Ready. Drop files or DICOM folders, pick a task, and Compute. All image processing runs locally, your images never leave the tab."
);
...
async function setupExamples() {
  ...
  const selector = createExampleSelector({
    ...
    onLoad: async (example, { fetchFiles, assertCurrent }) => {
      if (running)
        throw new Error("Stop processing before loading an example.");
      const files = await fetchFiles();
      assertCurrent();
      const records = await Promise.all(
        files.map(async (file) =>
          readImageRecord(file, await file.arrayBuffer())
        )
      );
      assertCurrent();
      if (running)
