...
const makeSetup = new Function('renderExampleSelector', 'fetch', 'document', '$', 'handleFiles', 'collectFromPicker', 'S', 'setStatus', `return async function() {${setupBody}`);
...
test('example selection passes the hosted volume through the normal ingest path', async () => {
  ...
  const setup = makeSetup(value => { options = value; return { root: {} }; },
    ...
    () => ({ before() {} }), async files => { imported = files; }, files => files,
    { volume: {} }, () => {});
  await setup();
