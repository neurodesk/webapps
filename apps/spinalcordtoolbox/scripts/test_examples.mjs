...
const source = await readFile(new URL('../web/js/spinalcordtoolbox-app.js', import.meta.url), 'utf8');
...
async function loadAdapter() {
  let options;
  const loaded = [];
  const app = { updateOutput() {}, fileIOController: { clearFiles() {}, handleFiles(files) { loaded.push(...files); } }, inputReady: Promise.resolve() };
  const selector = {};
  const scope = {};
  const input = { closest: () => ({ prepend(element) { assert.equal(element, selector); } }), classList: { remove() {} } };
  const document = { baseURI: 'https://example.org/spinalcordtoolbox/', getElementById: () => input, querySelector: selector => { assert.equal(selector, '.app-container'); return scope; } };
