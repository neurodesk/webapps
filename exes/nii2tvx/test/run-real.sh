...
"$root/nii2tvx" "$ref"/examples2/wM2017_T2w_lesion.nii.gz "$ref"/examples2/wM2018_T2w_lesion.nii.gz \
...
if [ -f "$root/nii2tvx.mjs" ] && command -v node >/dev/null 2>&1; then
  for subject in wM2017 wM2018 wM2208; do
    node "$root/wasm_demo.mjs" "$ref/examples2/${subject}_T2w_lesion.nii.gz" "$atlas" > "$work/$subject.tsv"
    "$root/nii2tvx" "$ref/examples2/${subject}_T2w_lesion.nii.gz" "$atlas" > "$work/$subject.native.tsv"
