...
const run = (command, args, options = {}) => {
  ...
  return execFileSync(command, args, { cwd: root, stdio: 'inherit', killSignal: 'SIGKILL', ...options });
};
...
run('docker', ['run', '--rm', '--network=none', '--shm-size=2g', '-v', `${root}:${root}`, '-v', `${process.execPath}:/usr/local/bin/node:ro`, '-w', root, ...env.flatMap(value => ['-e', value]), '--entrypoint', '/usr/bin/tini', image, '-s', '--', '/usr/bin/xvfb-run', '-a', '/usr/local/bin/node', 'scripts/desktop/smoke.mjs']);
