...
for (const modelsIncluded of [false, true]) {
  for (const platform of ['macos-arm64', 'windows-x64', 'linux-x64', 'linux-x64-apptainer']) {
    const download = suite.downloads.find(item => item.platform === platform && item.modelsIncluded === modelsIncluded);
    if (!download) throw new Error(`Missing ${modelsIncluded ? 'model-inclusive' : 'without-models'} ${platform} release`);
